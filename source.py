...
SETTINGS_TO_CHECK = ['source', 'youseetv.category', 'xmltv.type', 'xmltv.file', 'xmltv.url', 'xmltv.logo.folder', 'ottplaylist.ottServer1', 'ottplaylist.ottkey']
...
class Database(object):
    ...
    def setRequestForClose(self,value):
        self.requestForClose=value
    ...
    def _isCacheExpired(self, date):
        ...
        #test niklas
        c = self.conn.cursor()
        maxupdated=datetime.datetime.fromtimestamp(0)
        c.execute('SELECT max(programs_updated) as programs_updated FROM updates WHERE source=?', [self.source.KEY])
        row = c.fetchone()
        if row:
            maxupdated = row['programs_updated']

        return self.source.isUpdated(channelsLastUpdated, programsLastUpdated,date,maxupdated)
    ...
    def _updateChannelAndProgramListCaches(self, date, progress_callback, clearExistingProgramList):
        ...
        try:
            ...
            for item in self.source.getDataFromExternal(date, progress_callback):
                imported += 1
                print 'Imported_saetze:'+str(imported)
                if imported % 10000 == 0:
                    self.conn.commit()
                ...
                if isinstance(item, Channel):
                    imported_channels += 1
                    channel = item
                    #tt=unicode(channel.title)
                    #dd=channel.title.decode('utf-8', 'ignore')
                    c.execute(
                        ...
                        [channel.id, channel.title.decode('utf-8', 'ignore'), channel.logo, channel.streamUrl, channel.visible, channel.weight,
                    ...
                    if not c.rowcount:
                        c.execute(
                            ...
                            [channel.title.decode('utf-8', 'ignore'), channel.logo, channel.streamUrl, channel.weight, channel.visible,
...
class Source(object):
    ...
    def isUpdated(self, channelsLastUpdated, programsLastUpdated,viewTime, maxUpdated):
        today = datetime.datetime.now()
...
class XMLTVSource(Source):
    ...
    def __init__(self, addon):
        ...
        self.ottserverurl=[]
        if self.ottServer1 is not None:
            self.ottserverurl.append(self.ottServer1)
        if self.ottServer2 is not None:
            self.ottserverurl.append(self.ottServer2)
        if self.ottServer3 is not None:
            self.ottserverurl.append(self.ottServer3)

        self.externalPlaylist=None
        if len(self.ottserverurl)==0 or self.ottKey is None:
            raise SourceNotConfiguredException()
        #if (not self.xmltvFile or not xbmcvfs.exists(self.xmltvFile)) and not self.xmltvUrl:
         #   raise SourceNotConfiguredException()

    def getOTTPlalistURL(self,serverlink):
        return serverlink+'/playlist/'+self.ottKey+'/m3u'
    
    def getOTTTVGuide(self,tvguideurl):
        response = urllib2.urlopen(tvguideurl,timeout=7)
        guideresp = StringIO.StringIO()
        guideresp.write(response.read())
        guideresp.seek(0)
        response.close()
        jsondata=guideresp.read()
        jsondata=jsondata[jsondata.find('(')+1:].rstrip()[:-1]
    ...
    def getDataFromExternal(self, date, progress_callback=None):
        ...
        xmltv_file=None
        '''
        if self.xmltvType == XMLTVSource.TYPE_URL:
            if self.xmltvUrl.find('.gz')!=-1 or self.xmltvUrl.find('.gzip') != -1:
                # open and read GZip-XML http-Stream 
                response = urllib2.urlopen(self.xmltvUrl,timeout=30)
                compressedFile = StringIO.StringIO()
                compressedFile.write(response.read())
                compressedFile.seek(0)
                decompressedFile = gzip.GzipFile(fileobj=compressedFile, mode='rb')
                xmltv_file = decompressedFile.read()
                f = StringIO.StringIO(xmltv_file)
            else: 
                # open and read XML http-Stream              
                u = urllib2.urlopen(self.xmltvUrl, timeout=30)
                xmltv_file = StringIO.StringIO()
                xmltv_file.write(u.read())
                xmltv_file.seek(0) 
                u.close()
                f=xmltv_file
            ...
            size = f.size
        '''
        #if self.ottplaylistType is not None and int(self.ottplaylistType)==0:
        ...
        #else:
        #    self.externalPlaylist=None
        #parsedxmltvlist=self.parseXMLTV(context, f, size, self.logoFolder, progress_callback,self.externalPlaylist)
    ...
    def parseOTTXMLTV(self, logoFolder, progress_callback):
        ...
        for ottchannel in self.externalPlaylist:
            result=None
            result = Channel(ottchannel.chottid, ottchannel.title, ottchannel.logo, ottchannel.path, True)
            if result:
                yield result
            ...
            while content_loaded==False and i<3:
                try:
                    currentserverurl=self.ottserverurl[i]     
                    tvguideurl=currentserverurl+'/api/channel/'+ottchannel.chottid+'.jsonp'
                    tvguidejson=self.getOTTTVGuide(tvguideurl)
                    break
                except:
                    i=i+1
            
            if tvguidejson is not None:
                for programmdata in tvguidejson['epg_data']:
                    channel = ottchannel.chottid
                    description = programmdata['descr']
                    icon = None
                    if not description:
                        description = strings(NO_DESCRIPTION)
                    result = Program(channel, programmdata['name'], self.parseXMLTVDate(programmdata['time']),
                                     self.parseXMLTVDate(programmdata['time_to']), description, imageSmall=icon)
                    
                    if result:
                        yield result

        
    def isUpdated(self, channelsLastUpdated, programLastUpdate,viewTime, maxUpdated):

        today = datetime.datetime.now()
        todaynew=today - datetime.timedelta(hours=2)
        # wenn channels nicht upgedated wurden dann update erzwingen
        if channelsLastUpdated is None:
            return True
        #wenn view in die Vergangenheit anzeigt, dann kein update notwendig
        if viewTime<todaynew:
            return False
        #after24hours=datetime.datetime.today() -datetime.timedelta(hours=1)
        after24hours=datetime.datetime.today()+datetime.timedelta(hours=int(self.otttimeshift)) 
        #if maxUpdated is not None:
        #    maxUpdated=dateFromUnixtime(maxUpdated)
        #xbmc.log("CH-UPDATE Loop: viewTime" + str(viewTime), level=xbmc.LOGDEBUG)  
        #xbmc.log("CH-UPDATE Loop: maxUpdated" + str(maxUpdated), level=xbmc.LOGDEBUG)  
        #xbmc.log("CH-UPDATE Loop: today" + str(today), level=xbmc.LOGDEBUG) 
        #xbmc.log("CH-UPDATE Loop: todaynew" + str(todaynew), level=xbmc.LOGDEBUG)
        #xbmc.log("CH-UPDATE Loop: channelsLastUpdated" + str(channelsLastUpdated), level=xbmc.LOGDEBUG)    
        #xbmc.log("CH-UPDATE Loop: after24hours" + str(after24hours), level=xbmc.LOGDEBUG)  
        #xbmc.log("CH-UPDATE Loop: programLastUpdate" + str(programLastUpdate), level=xbmc.LOGDEBUG) 
        #xbmc.log("CH-UPDATE Loop: programLastUpdate<after24hours" , level=xbmc.LOGDEBUG)
        if programLastUpdate== datetime.datetime.fromtimestamp(0):
            return True
        else: return False
        #return programLastUpdate<after24hours
        #
        #
        #
        #
    ...
    def parseXMLTVDate(self, dateString):
        if dateString is not None:
            ...
            #if not dateString.isdigit() and dateString.find(' ') != -1:               
            #if dateString.find(' ') != -1:
    ...
    def parseXMLTV(self, context, f, size, logoFolder, progress_callback, externalPlaylist):
        ...
        for event, elem in context:
            if event == "end":
                ...
                if elem.tag == "programme":
                    ...
                    #if externalPlaylist is not None:
                    #    for ottchannel in playlist:
                    #        if ottchannel.title.lower()==title.lower():
                    #           ottchannel.chid=id;
                    #           ottchannel.logo=logo
                    #        else 
                    #           cutpoint=ottchannel.path.find('/stream')
                    #           ottchannel.logo=song.path[0:cutpoint:]+'/images/'+ottchannel.title
                    #        print (track.title, track.length, track.path, track.chid, track.chottid, track.logo)
                    #    #xbmc.log(msg='Raven startlog 7:'+str(url), level=xbmc.LOGNOTICE)
                    #        # fileread
                    #        #search channel ect.
