...
if __name__ == '__main__':
    ...
    #Service ausfuehren 
    try:
        ADDON = xbmcaddon.Addon(id = 'script.tvglobus')
        if ADDON.getSetting('cache.data.on.xbmc.startup') == 'true':
            Service()
    except source.SourceNotConfiguredException:
        pass  # ignore
    except Exception, ex:
        xbmc.log('[script.tvglobus] Uncaugt exception in service.py: %s' % str(ex) , xbmc.LOGDEBUG)
    ...
    while not monitor.abortRequested():
        ...
        if monitor.waitForAbort(10):
            ...
            break
        xbmc.log("Monitor Thread: waiting for kodis end -> %s" % time.time(), level=xbmc.LOGDEBUG)
